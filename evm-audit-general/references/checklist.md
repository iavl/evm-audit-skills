...
- [ ] **[EVM-GEN-097] Read-After-Call for Security Decisions** _(exploit-pattern; medium)_: A variable read after an external call is used for access control, balance checking, or amount calculation. The callee can manipulate that state during the call.
  ...
  - **FP:** No finding when the source checklist's required invariant or validation is enforced on every reachable path and attacker-controlled inputs cannot trigger the described condition.
  - **Proof:** For every external call, audit what is read after. Any security-critical read after an external call must either be re-validated or moved before the call.
...
- [ ] **[EVM-GEN-098] Atomic Value Transfer** _(exploit-pattern; medium)_: Sender's balance decreases by X but receiver's increases by Y != X (minus fees).
  ...
  - **FP:** No finding when the source checklist's required invariant or validation is enforced on every reachable path and attacker-controlled inputs cannot trigger the described condition.
  - **Proof:** For each transfer, verify source debit == destination credit + documented fee.
...
- [ ] **[EVM-GEN-099] No Free Extraction** _(exploit-pattern; medium)_: A sequence of calls allows withdrawing more value than deposited (net of fees and yield).
  ...
  - **FP:** No finding when the source checklist's required invariant or validation is enforced on every reachable path and attacker-controlled inputs cannot trigger the described condition.
  - **Proof:** Model the protocol as a closed economy. Attempt to construct a cycle that produces profit without external input.
